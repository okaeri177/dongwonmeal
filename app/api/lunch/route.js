// Next.js 14 App Router 방식의 POST 요청 처리 함수입니다.
export async function POST(request) {
  // 카카오톡 챗봇이 보내준 JSON 데이터를 읽어옵니다.
  ...
  async function getMeal(ymd, mealCode) {
    ...
    const url = `https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=${apiKey}&Type=json&ATPT_OFCDC_SC_CODE=S10&SD_SCHUL_CODE=9010463&MLSV_YMD=${ymd}&MMEAL_SC_CODE=${mealCode}`;
  ...
  if (params.date) {
    ...
    msg = `🍱 동원고 ${month}월 ${day}일 (${dayNames[target.getDay()]}) 급식\n─────────────\n🌅 조식\n${breakfast}\n─────────────\n🌞 점심\n${lunch}\n─────────────\n🌙 석식\n${dinner}`;
    ...
    const mon = dates[0];
    const fri = dates[4];
    msg = `🍱 동원고 이번주 ${mealName} (${mon.getMonth() + 1}/${mon.getDate()}~${fri.getMonth() + 1}/${fri.getDate()})\n`;
    dates.forEach((d, i) => {
      msg += `─────────────\n${emoji} ${dateLabel(d)}\n${results[i]}\n`;
    });
    ...
    const dayNames = ["일", "월", "화", "수", "목", "금", "토"];
    msg = `🍱 동원고 ${target.getMonth() + 1}월 ${target.getDate()}일 (${dayNames[target.getDay()]}) 급식\n─────────────\n🌞 점심\n${lunch}\n─────────────\n🌙 석식\n${dinner}`;
  }

  // Next.js 표준 방식으로 카카오톡 규격의 JSON 응답을 반환합니다.
